...
vbuffer *model_load_from_object(char *filename, char *model_name, float r, float g, float b, float a)
{
	...
	while (read_line(file, line) != -1)
	{
		...
		else if (strcmp(terms[0], "f"))
		{
			char *data[3];
			vertex_number += 3;
			vbuff->data = realloc(vbuff->data, vertex_number * sizeof(float));
